...
try:
    from micro_web_template import MicroWebTemplate
except:
    pass

try:
    from micro_web_socket import MicroWebSocket
except:
    pass
        ...
        def ReadRequestPostedFormData(self):
            ...
            if len(data) > 0:
                ...
                for s in elements:
                    ...
                    if len(param) > 0:
                        value = MicroWebSrv._unquote(param[1]) if len(param) > 1 else ''
                        res[MicroWebSrv._unquote(param[0])] = MicroWebSrv._unquote_plus(value)
            return res
